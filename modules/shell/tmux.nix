{pkgs, ...}: {
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      bind -n M-k run-shell "if $is_vim ; then tmux send-keys A-k; else tmux select-pane -D; fi"
      bind -n M-j run-shell "if $is_vim ; then tmux send-keys A-h; else tmux select-pane -L; fi"
      bind -n M-i run-shell "if $is_vim ; then tmux send-keys A-i; else tmux select-pane -U; fi"
      bind -n M-l run-shell "if $is_vim ; then tmux send-keys A-l; else tmux select-pane -R; fi"

      bind -n C-M-k run-shell "if $is_vim ; then tmux send-keys M-k; else tmux resize-pane -D; fi"
      bind -n C-M-j run-shell "if $is_vim ; then tmux send-keys M-h; else tmux resize-pane -L; fi"
      bind -n C-M-i run-shell "if $is_vim ; then tmux send-keys M-i; else tmux resize-pane -U; fi"
      bind -n C-M-l run-shell "if $is_vim ; then tmux send-keys M-l; else tmux resize-pane -R; fi"
